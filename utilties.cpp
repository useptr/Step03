...
// Out :
// AcDbObjectId& arCLayerId : ObjectId of the created or existing layer
//
Acad::ErrorStatus CreateLayer(const TCHAR* aszLayerName, AcDbObjectId& arCLayerId) {

	arCLayerId = AcDbObjectId::kNull;
	...
	if (playerTable->getAt(aszLayerName, arCLayerId) == Acad::eOk) {
	...
	es = playerTable->add(arCLayerId, pLayerTableRecord);
	if (es != Acad::eOk) { // release resources
